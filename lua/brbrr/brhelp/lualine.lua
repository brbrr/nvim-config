---@class BrHelp.lualine
local M = {}

---@param icon string
---@param statusFn fun(): nil|"ok"|"error"|"pending"
function M.status(icon, statusFn)
  local colors = {
    ok = 'Special',
  ...
  return {
    ...
    color = function()
      return { fg = Snacks.util.color(colors[statusFn()] or colors.ok) }
    end,
  }
end
...
function M.cmp_source(name, icon)
  icon = icon or BrHelp.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = BrHelp.root.get { normalize = true }
    local cwd = BrHelp.root.cwd()
