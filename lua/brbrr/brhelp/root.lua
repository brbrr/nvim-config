...
---@class BrHelpRoot
---@field paths string[]
---@field spec BrHelpRootSpec

---@alias BrHelpRootFn fun(buf: number): (string|string[])

---@alias BrHelpRootSpec string|string[]|BrHelpRootFn
...
function M.detectors.lsp(buf)
  ...
  local clients = BrHelp.lsp.get_clients { bufnr = buf }
...
function M.realpath(path)
  ...
  return BrHelp.norm(path)
end

---@param spec BrHelpRootSpec
---@return BrHelpRootFn
...
---@param opts? { buf?: number, spec?: BrHelpRootSpec[], all?: boolean }
function M.detect(opts)
  opts = opts or {}
  opts.spec = opts.spec or type(vim.g.root_spec) == 'table' and vim.g.root_spec or M.spec
  opts.buf = (opts.buf == nil or opts.buf == 0) and vim.api.nvim_get_current_buf() or opts.buf

  local ret = {} ---@type BrHelpRoot[]
...
function M.info()
  ...
  lines[#lines + 1] = '```'
  BrHelp.info(lines, { title = 'BrHelp Roots' })
...
function M.setup()
  vim.api.nvim_create_user_command('BrHelpRoot', function()
    BrHelp.root.info()
  end, { desc = 'BrHelp roots for the current buffer' })
